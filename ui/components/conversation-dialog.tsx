          ...
          {log &&
            (messages.length > 0 ? (
              messages.map((msg, idx) => (
                <div key={idx} className="glass rounded-xl p-4 border border-[var(--border-soft)]">
                  <div className="flex items-center gap-2 text-sm text-[var(--ink-700)] mb-2">
                  ...
                  <div className="text-sm leading-relaxed text-[var(--ink-700)] whitespace-pre-wrap">{msg.content}</div>
                </div>
              ))
            ) : (
